var initDjangoHStoreGrappelliWidget = function(hstore_field_name) {

    $ = django.jQuery;

    var hstore_field_id = 'id_'+hstore_field_name,
        original_textarea = $('#'+hstore_field_id),
        original_container = original_textarea.parents('.grp-row'),
        ...
        hstore_field_data = {
            ...
            'help': original_container.find('.grp-help').text(),
            'data': json_data
        },
        // compile template
    ...
    // remove row link
    $hstore.delegate('a.grp-remove-handler', 'click', function(e) {
        e.preventDefault();
        // cache container jquery object before $(this) gets removed
        var container = $(this).parents('.hstore');
        $(this).parents('.grp-row').remove();
        updateTextarea(container);
    });
    
    //// add row link
    $hstore.delegate('.grp-add-handler, .grp-add-link', 'click', function(e) {
        e.preventDefault();
    ...
    $hstore.delegate('.hstore-toggle-txtarea', 'click', function(e) {
        ...
        var container = $(this).parents('.hstore'),
            ...
            add_row = container.find('.grp-add-link').parent();
